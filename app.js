...
function localHydrantMatches(q){
  ...
  if(!n)return [];
  return points.filter(p=>normalizeText(`${p.name} ${p.address||""}`).includes(n)).slice(0,5);
}
...
function renderUnifiedResults(container, query, addresses){
  ...
  container.querySelectorAll("button").forEach(btn=>btn.onclick=()=>{
    ...
    if(x.kind==="point"){const p=points.find(v=>v.id===x.id);map.setView([p.lat,p.lng],18);showDetails(p.id);return}
    pendingLatLng={lat:x.lat,lng:x.lng};
...
$("coverageBtn").onclick=()=>{
  ...
  const center=selected?[selected.lat,selected.lng]:map.getCenter();
...
function nearestHydrantTo(latlng){
  return points.filter(p=>p.type==="hydrant"&&p.status!=="out").map(p=>({...p,distance:distanceMeters(latlng,[p.lat,p.lng])})).sort((a,b)=>a.distance-b.distance)[0];
}
$("incidentBtn").onclick=()=>{
  $("sidePanel").classList.remove("open");$("incidentPanel").classList.remove("hidden");$("incidentAddress").focus();
};
$("closeIncidentBtn").onclick=()=>$("incidentPanel").classList.add("hidden");
$("incidentAddress").oninput=()=>{
  ...
  addressTimer=setTimeout(async()=>{
    ...
    $("incidentAddressResults").querySelectorAll("button").forEach(b=>b.onclick=()=>{
      ...
      selected=h;$("incidentHydrant").textContent=h.name;$("incidentDistance").textContent=`${Math.round(h.distance)} m`;
      $("incidentFlow").textContent=flowLabel(h);
...
$("incidentNavigateBtn").onclick=()=>{
  if(!selected||!incidentLocation){toast("Choisissez d’abord une adresse");return}
  window.open(`https://www.google.com/maps/dir/?api=1&origin=${incidentLocation[0]},${incidentLocation[1]}&destination=${selected.lat},${selected.lng}`,"_blank");
};
...
$("dismissInstallBtn").onclick=()=>$("installBanner").classList.add("hidden");
